...
bool handle_history_request(const IpcHandlerContext& context, std::string* response_json)
{
    ...
    if (method == "get_device_history_view") {
        ...
        if (respond_if_error(request_status, id_json, request_error.empty() ? "设备历史查询请求参数非法" : request_error, response_json)) return true;

        DeviceHistoryView view;
        std::string error_message;
        const auto status = backend_service_->get_device_history_view(query, &view, &error_message);
        if (respond_if_error(status, id_json, error_message.empty() ? "历史数据页面加载失败" : error_message, response_json)) return true;
    ...
    if (method == "get_history_overview_summaries") {
        ...
        if (respond_if_error(status, id_json, error_message.empty() ? "历史总览摘要获取失败" : error_message, response_json)) return true;
    ...
    if (method == "export_history_records") {
        ...
        if (respond_if_error(status, id_json, error_message.empty() ? "历史数据导出查询失败" : error_message, response_json)) return true;
    ...
    if (method == "get_data_maintenance_summary") {
        ...
        if (respond_if_error(status, id_json, error_message.empty() ? "数据维护摘要获取失败" : error_message, response_json)) return true;
    ...
    if (method == "cleanup_expired_data") {
        ...
        if (respond_if_error(status, id_json, error_message.empty() ? "超期数据清理失败" : error_message, response_json)) return true;
        *response_json = ipc_protocol::build_success_response(id_json, ipc_json::to_json(summary));
        return true;
    }
