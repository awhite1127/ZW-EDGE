...
const DeviceTemplateDefinition& r4_rd_1000h_device_template()
{
    static const DeviceTemplateDefinition value = make_builtin_single_read_block_template(
        "R4-RD-1000H",
        "R4接地电阻值",
        "R4 1000H 基础数据值区，单点 1 寄存器，接地电阻按两位小数解析，单次读取寄存器数量建议不超过 70。原始值乘以 0.01；内置双字节数值状态由统一模块判定。R4 参数区从 0xF100/0xF200 开始，本模板不维护参数；参数维护请依据厂家工具或完整维护规约执行。",
        4096,
        1,
        {
            r4_resistance_field(),
...
const DeviceTemplateDefinition& r4_wt_1000h_device_template()
{
    static const DeviceTemplateDefinition value = make_builtin_single_read_block_template(
        "R4-WT-1000H",
        "R4无线测温值",
        "R4 1000H 基础数据值区，单点 1 寄存器，温度按 raw 减 400 后再除以 10 解析，单次读取寄存器数量建议不超过 70。温度 = (原始值 - 400) / 10；内置双字节数值状态由统一模块判定。R4 参数区从 0xF100/0xF200 开始，本模板不维护参数；参数维护请依据厂家工具或完整维护规约执行。",
        4096,
        1,
        {
