...
#include "service/polling_service_internal.h"
#include <chrono>
#include <cmath>
#include <ctime>
#include <exception>
    ...
    const std::vector<DeviceStatus>& statuses) const
{
    ...
    if (data_store_ != nullptr) {
        std::vector<DeviceId> failed_ids;
        failed_ids.reserve(statuses.size());
        for (const auto& status : statuses) {
            if (!status.last_collect_success) failed_ids.push_back(status.device_id);
        }
        const auto cached_statuses = data_store_->get_device_statuses(failed_ids);
