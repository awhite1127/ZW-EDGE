    ...
    bool invalid_at_65535 = false)
{
    return make_builtin_scalar_field(
        ...
        history_enabled,
        display_order,
        invalid_at_65535 ? "greater_or_equal" : "none",
        invalid_at_65535 ? 65535.0 : 0.0,
        0.0,
        0.0);
}
...
const DeviceTemplateDefinition& default_device_template()
{
    static const DeviceTemplateDefinition value = make_builtin_single_read_block_template(
        ...
        "接地电阻",
        "RD100 接地电阻采集模板；原始值大于等于 0xFFF0 时表示数据无效或设备故障。",
        4096,
        1,
        {
...
const DeviceTemplateDefinition& communication_manager_rd100_device_template()
{
    static const DeviceTemplateDefinition value = make_builtin_single_read_block_template(
        "CM-RD100",
        "通讯管理机-RD100",
        "通讯管理机映射 RD100 接地电阻监测点模板；原始值大于等于 0xFFF0 时表示数据无效或设备故障。",
        4096,
        1,
        {
            rd100_resistance_field(),
...
const DeviceTemplateDefinition& em100_insulation_monitor_device_template()
{
    static const DeviceTemplateDefinition value = [] {
        auto result = make_builtin_single_read_block_template(
            ...
            40,
            {
                em100_uint16_field("insulation_resistance", "绝缘电阻值", 0, 1.0, "MΩ", 0, true, true, true, 1, true),
                em100_uint16_field("residual_current", "剩余电流值", 1, 1.0, "mA", 0, true, true, true, 2, true),
                em100_uint16_field("absorption_ratio", "吸收比", 2, 0.01, "", 2, true, true, true, 3, true),
                em100_uint16_field("polarization_index", "极化指标", 3, 0.01, "", 2, true, true, true, 4, true),
